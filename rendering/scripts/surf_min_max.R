...
# get min and max values to replace
minv <- min(vals$V2)
maxv <- max(vals$V2)

# write text files 
write.table(minv, paste(ostem,"_min",sep=""), quote=F, row.names=F, col.names=F)
write.table(maxv, paste(ostem,"_max",sep=""), quote=F, row.names=F, col.names=F)
